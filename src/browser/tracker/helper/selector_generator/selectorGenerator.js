...
import { getAccessibleName, getElementRole, queryShadowAll, isVisible } from '../dom/domUtils.js';

/**
 * Tạo Playwright Locator Code (Output giống hệt Codegen)
 */
export function generateAndValidateSelectors(element, options = {}) {
  // 1. Tạo danh sách các chiến lược định danh (Candidates)
  const candidates = generateCandidates(element);

  // 2. Kiểm tra tính duy nhất (Uniqueness Check)
  // Ưu tiên theo thứ tự trong mảng candidates (TestID > Role > Text...)
  ...
  // 3. Nếu không tìm thấy single locator duy nhất, thử chiến lược Chaining (Parent >> Child)
  const chained = tryChaining(element);
  if (chained) {
    return [chained];
  }

  // 4. Fallback: CSS Selector tốt nhất có thể
...
function generateCandidates(element) {
    ...
    if (element.hasAttribute(attr)) {
      ...
      if (value) {
        candidates.push({
          ...
          attr: attr,
          value: value,
          // Selector dùng để validate uniqueness
  ...
  // B. Get By Role (Ưu tiên số 2 - "The Playwright Way")
  const role = getElementRole(element);
  const name = getAccessibleName(element);
  ...
  if (role) {
    const roleCandidate = {
      ...
      // Với role, việc validate phức tạp hơn vì querySelector không support role
      // Chúng ta sẽ dùng css attribute approximation để validate sơ bộ hoặc logic riêng
      queryRole: true 
    };
    candidates.push(roleCandidate);
  }

  // C. Get By Label (Form inputs)
  if (['input', 'textarea', 'select'].includes(tag)) {
    // Label logic nằm trong domUtils, ở đây ta kiểm tra label text
    // Giả sử ta tìm được text của label liên kết
    const labelText = getAccessibleName(element); // Hàm này đã cover label linkage
    if (labelText) {
      candidates.push({
        type: 'label',
        text: labelText,
    ...
    if (placeholder) {
      candidates.push({
        type: 'placeholder',
        text: placeholder,
        query: `${tag}[placeholder="${escapeSelector(placeholder)}"]`
      });
    }
  }

  // D. Get By Text (Nội dung hiển thị)
  // Chỉ áp dụng cho các element chứa text trực tiếp hoặc interactive
...
/**
 * Kiểm tra xem candidate locator có trỏ đến duy nhất element mục tiêu không
 */
function isUnique(candidate, targetElement) {
  let matches = [];

  // Logic validate tùy theo loại candidate
  if (candidate.query) {
    // CSS Selector thông thường
    matches = queryShadowAll(candidate.query);
  } 
  else if (candidate.queryRole) {
    // Mô phỏng logic tìm Role
    // Tìm tất cả element, filter xem cái nào khớp Role & Name
    ...
    // Mô phỏng tìm Text
    // Playwright match text khá lỏng (contains), nhưng strict mode cần exact
    const allElements = queryShadowAll('*');
    matches = allElements.filter(el => {
        // Chỉ xét element chứa text node trực tiếp hoặc là leaf
        return el.textContent.includes(candidate.text) && isVisible(el);
    });
    // Lọc lại exact match nếu cần
    ...
    if (exactMatches.length > 0) matches = exactMatches;
     ...
     matches = inputs.filter(el => getAccessibleName(el) === candidate.text);
  }
...
/**
 * Thử chiến lược Chaining (Parent lọc Child)
 * Ví dụ: page.locator('.card').filter({ hasText: 'Iphone' }).getByRole('button')
 */
function tryChaining(element) {
  // Logic đơn giản: Tìm cha có CSS class/id tốt
  let parent = element.parentElement;
  let depth = 0;
  
  while (parent && depth < 3) {
    // Tạo selector đơn giản cho parent
    let parentSelector = null;
    if (parent.id) parentSelector = `#${escapeSelector(parent.id)}`;
    else if (parent.classList.length) {
       // Lấy class đầu tiên không phải ephemeral
       const validClass = [...parent.classList].find(c => !c.startsWith('ng-') && !c.includes('active'));
       if (validClass) parentSelector = `.${escapeSelector(validClass)}`;
    }

    if (parentSelector) {
        // Kiểm tra parent selector có unique không
        const parentMatches = queryShadowAll(parentSelector);
        if (parentMatches.length === 1) {
             // Parent Unique! Giờ tìm đường dẫn từ Parent -> Child
             // Fallback đơn giản: dùng CSS tag từ parent
             const tag = element.tagName.toLowerCase();
             // Code: page.locator('#parent-id').locator('button')
             return `page.locator('${parentSelector}').locator('${tag}')`; 
             // Lưu ý: Cần thêm logic nth nếu con trong parent không unique, nhưng tạm thời thế này là ổn.
        }
    }
...
/**
 * Chuyển đổi Candidate Object thành Code Playwright
 */
function candidateToCode(candidate) {
  switch (candidate.type) {
    case 'testid':
      // Nếu attribute mặc định là data-testid thì dùng getByTestId, không thì dùng locator
      if (candidate.attr === 'data-testid') {
        return `page.getByTestId('${candidate.value}')`;
      }
      return `page.locator('[${candidate.attr}="${candidate.value}"]')`;
      
    case 'role':
      const roleName = candidate.name 
        ? `, { name: '${candidate.name.replace(/'/g, "\\'")}' }` 
        : '';
      return `page.getByRole('${candidate.role}'${roleName})`;
      
    case 'text':
      return `page.getByText('${candidate.text.replace(/'/g, "\\'")}')`;
      
    case 'label':
      return `page.getByLabel('${candidate.text.replace(/'/g, "\\'")}')`;
      
    case 'placeholder':
      return `page.getByPlaceholder('${candidate.text.replace(/'/g, "\\'")}')`;
      
    case 'alt':
      return `page.getByAltText('${candidate.text.replace(/'/g, "\\'")}')`;
      
    case 'title':
      return `page.getByTitle('${candidate.text.replace(/'/g, "\\'")}')`;
