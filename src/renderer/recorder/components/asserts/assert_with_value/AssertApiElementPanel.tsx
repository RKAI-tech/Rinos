...
interface AssertApiElementPanelProps {
...
const AssertApiElementPanel: React.FC<AssertApiElementPanelProps> = ({
  apiRequest,
  apiResponse,
  onChange,
  onSendRequest,
  ...
  const buildApiRequestData = (opts: { forSave: boolean }): ApiRequestData => {
    ...
    const includeBasicStorage =
      authType === 'basic' &&
      basicAuthStorageEnabled &&
      basicAuthUsernameKey.trim() &&
      basicAuthPasswordKey.trim();
  ...
  const handleFetchToken = async () => {
    try {
      ...
      const source =
        tokenStorageType === 'localStorage' ? 'local' : tokenStorageType === 'sessionStorage' ? 'session' : 'cookie';
  ...
  const handleSendRequest = async () => {
    ...
    const options: ApiRequestOptions = convertApiRequestDataToOptions(runtimeApiData);
    ...
    try {
      ...
      // Call onSendRequest to update parent with apiData and response
  ...
  return (
    <div className="aiam-db-box" style={{ padding: '12px' }}>
      ...
      <div className="aep-method-url">
        <select className="aep-method-select" value={method} onChange={e => setMethod(e.target.value)}>
          {httpMethods.map(m => (
        ...
        />
        <button className="aep-send-btn" onClick={handleSendRequest} disabled={isSending || !isValidUrl}>
        ...
        <div className="aep-params">
          {params.map((param, index) => (
            <div key={index} className="aep-param-row">
              ...
              />
              <button className="aep-remove-btn" onClick={() => handleRemoveParam(index)}>
                ✕
              </button>
            </div>
          ))}
        ...
        <div className="aep-headers">
          {headers.map((header, index) => (
            <div key={index} className="aep-header-row">
              ...
              />
              <button className="aep-remove-btn" onClick={() => handleRemoveHeader(index)}>
                ✕
              </button>
            </div>
          ))}
      ...
      {(tokenStorageEnabled || basicAuthStorageEnabled) && authType !== 'none' && (
        <div className="aep-section">
          <div className="aep-section-header">
            <span className="aep-section-title">
          ...
          <div className="aep-token-storage">
            ...
            <div className="aep-storage-key">
              <label className="aep-label">
                {tokenStorageType === 'cookie' ? 'Cookie Name' : 'Storage Key'}{' '}
          ...
          <div className="aep-token-storage">
            ...
            {basicAuthStorageType === 'localStorage' || basicAuthStorageType === 'sessionStorage' ? (
              <div className="aep-basic-auth-keys">
                <div className="aep-storage-key">
                  <label className="aep-label">
                    Username Key <span style={{ color: 'red' }}>*</span>
                  </label>
                  <input
                    className="aep-storage-input"
                    type="text"
                ...
                <div className="aep-storage-key">
                  <label className="aep-label">
                    Password Key <span style={{ color: 'red' }}>*</span>
                  </label>
                  <input
                    className="aep-storage-input"
                    type="text"
        ...
        {bodyType === 'form' && (
          <div className="aep-params">
            ...
            <div className="aep-add-param">
              <button
                className="aep-add-btn"
                onClick={() => setBodyForm([...bodyForm, { key: '', value: '' }])}
              >
                + ADD FIELD
              </button>
            </div>
        ...
        <div className="aep-section">
          ...
          <div className="aep-response">
            <div className="aep-response-status">
              Status:{' '}
              <span
                className={`aep-status-code ${getStatusColorClass(apiResponse.status).replace('arm-', 'aep-')}`}
              >
                {apiResponse.status} {getStatusDescription(apiResponse.status)}
              </span>
            </div>
            <div className="aep-response-content">
              <pre className="aep-response-data">{formatResponseData(apiResponse.data)}</pre>
            </div>
          </div>
        </div>
      )}
...
export default AssertApiElementPanel;
