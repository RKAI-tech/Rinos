...
const ActionDetailModal: React.FC<ActionDetailModalProps> = ({ 
  ...
  // Hàm update tổng quát để các component con có thể tự quản lý việc update
  ...
  const handleSave = () => {
    if (draft && onSave) {
      ...
      // Use specialized normalize functions for input, navigate, click, select, change, and drag and drop
      if (draft.action_type === ActionType.input) {
        ...
        draft.action_type === ActionType.assert &&
        isAssertWithoutValueType(draft.assert_type)
      ) {
        ...
        // Use default normalize for other action types
    ...
    <div className="rcd-action-detail-overlay">
      <div className="rcd-action-detail-container" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="rcd-action-detail-content">
          {/* Render specialized components for input, navigate, click, select, and change */}
