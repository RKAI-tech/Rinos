...
export class BrowserVariableService {
    ...
    async getBrowserVariableValue(browserVariableId: string): Promise<ApiResponse<any>> {
        if (!browserVariableId) {
            return { success: false, error: 'browser_variable_id is required' };
        }
        return await apiRouter.request(`/browser-variables/${browserVariableId}/value`, {
            method: 'GET'
        });
    }
